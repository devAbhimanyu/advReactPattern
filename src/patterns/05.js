...
const Usage = () => {
  const MAX_USER_CLICKS = 20;
  const [state, setState] = useState(INITIAL_STATE);
  const clapHandler = () => {
    setState(({ count, totalCount }) => ({
      isClicked: true,
      count: Math.min(count + 1, MAX_USER_CLICKS),
      totalCount: count < MAX_USER_CLICKS ? totalCount + 1 : totalCount
    }));
  };
  return (
    <div>
      ...
      <div>{`You have clapped ${state.count} times`}</div>
    </div>
  );
};
